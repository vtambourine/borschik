    ...
    File: exports.File = INHERIT(cssbase.File, {
        ...
        parseInclude: function(content) {
            ...
            var allIncRe = new RegExp([
                    ...
                    .join('|')
                    + '|' +
                    // RegExp to find comments "/**/" "//" and borschik.link("path/to/image.png")
                    '\\/\\/.*|\\/\\*[\\s\\S]*?\\*\\/|borschik\\.link\\([\'"]([^@][^"\']+?)[\'"]\\)',
                    'g'),
            ...
            var includes = [],
                texts = content
                    ...
                    .replace(allIncRe, function(_, incCommFile, incStrFile, borschikLink) {
        ...
        processInclude: function(baseFile, content) {
            ...
            for(var i = 0; i < parsed.length; i++) {
                ...
                if (!FS.existsSync(item.file)) {
                    item.file = item.file.replace(/\/_([a-z0-9-_]*\.[a-z0-9]{2,3})/, function(match, filename) {
                        return '/' + filename;
                    });
                }
